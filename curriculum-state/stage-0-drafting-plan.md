...
>
> **Revised 2026-09-20.** This plan produced `ch01-foundations.md`, which the
> owner rejected as cold, dense and full of names and dates. The tables below
> are the reason: each lesson's only concrete content was an **Anchors** column
> of names and years, and each chapter carried an **Opens on** line naming its
> subject as the first move. `ch01` was drafted "matching the plan exactly" and
> came out exactly like the plan.
>
> Two corrections, and they outrank every table below:
>
> 1. **The Anchors column is backstage research, not the chapter's content.**
>    It is what the writer must be *correct about*. It is not a list of things
>    the reader must be *told*. Most anchors never reach the page. A date, a
>    name or an institution appears in the prose only where it is load-bearing
>    to the sentence carrying it — and in Stage 0 that is rare.
>    `canon/corrections.md` §4 and §11.
> 2. **The "Opens on" line is not the first thing on the page.** Every chapter
>    first welcomes the reader, links back, says what it covers **and why it
>    matters** — see "How every chapter opens" below. "Opens on" describes
>    where the *teaching* starts, after that.
>
> **The column that should drive a chapter is "The one idea it must land".**
> Build the lesson around that, find an everyday thing the reader can picture
> to carry it, and let the anchors keep you honest from backstage.

**Status: approved 2026-09-16. Nothing is drafted yet.** No Stage 0 MDX has been written against this plan and no `prerequisite-graph.yaml` rows have been added. The currently-shipped Stage 0 files (`edu-site/docs/intro-1-*` through `intro-5-*`) are the *old* Stage 0 and are being replaced from scratch — harvest-only for verified citations, never for prose. (Their openings, their density and their bold rhythm are all things this plan's chapters must *not* copy.)
...
Where a chapter's subject has roots older than its position (terminals descend from much older printing telegraphs), **the chapter says so plainly, early, in its own words** rather than being relocated. One sentence, in the teaching — not a dated preamble, and not in the welcome block.

**One deliberate exception, in the whole stage:** Chapter 18 rewinds to 1956. AI research ran as a parallel track that did not touch working programmers until roughly 2021, and splitting it across five earlier chapters would destroy a narrative that only works whole. It says plainly, early, that it is going back to the beginning of a story that ran in parallel. A date is allowed there because the rewind *is* the point — that is what load-bearing means. Every other chapter moves forward.

## How to read a chapter block below

- **Assumes** — what the reader already has. Do not re-teach it.
- **Opens on** — where the *teaching* starts, **after** the chapter's welcome and orientation block. Never the literal first thing on the page.
...
- **Anchors** — **backstage research.** The named things the writer must be correct about. Not a checklist to exhaust, not a list to put on the page, and not a licence to add more from memory. Anything not here gets verified in the dossier first.
- **The one idea it must land** — the column that actually drives the lesson. If a draft lands this and mentions three of the anchors, it is correct. If it lands all the anchors and not this, it has failed.

## On density — the rule this plan broke once

**Anchors are the researched backing, not the reader's diet.** Keep dates,
proper nouns and invented-thing counts low in the prose. Rigour stays backstage.

That sentence was already in this file when `ch01` was drafted, and it lost —
one line against twenty tables whose only concrete content was names and years.
So, concretely:

- **Before writing any date or proper noun into the prose, ask what the sentence
  loses without it.** If the answer is "nothing", it is furniture.
- **A chapter is not a timeline.** Stage 0 is ordered chronologically; that is a
  fact about the *order of the chapters*, not a licence for dates in sentences.
- **Every chapter needs one everyday thing a beginner can picture**, returned to
  as the chapter moves — not a chain of named historical people. Record it in
  `example-ledger.yaml`. "The subject is its own example" is not an answer;
  it is the reasoning that produced `ch01`, and it has been reversed.
- The later chapters in this plan are far denser than Chapter 1 — Chapter 5
  lists six editors with years, Chapter 14 lists five platforms with years.
  Written the way `ch01` was written, those become unreadable for a beginner.
  **Treat every anchor list from Chapter 3 onward as a research index.**

## How every chapter opens

Binding on every chapter drafted from this plan. Full rule:
`canon/corrections.md` §7.

1. **A real hello that places the reader.** Vary it — no welcome style returns
   until four or five chapters later.
2. **A link back** to what the previous chapter left them holding.
3. **What this chapter covers**, as a few grouped bullets.
4. **Why it matters** — what it unlocks. Not optional.

Then the teaching starts, at the "Opens on" line, **problem-first or
story-first**. No opening phrase is banned: "In this chapter we'll look at…"
and "Let's start" are both fine. The move from the opening into the first
section must be gradual — a warm welcome followed by a cold section heading is
the same failure, forty words later.
...
| Rule | Where it lives |
|---|---|
| Name-First spine, beginner reader model, language register | `.claude/skills/lesson-spine-authoring/` |
| **Standing corrections — read first, always** | `curriculum-state/canon/corrections.md` |
| Every chapter welcomes the reader, links back, covers what **and why** | `canon/corrections.md` §7 |
| Ten-angle beginner-experience audit before presenting | `lesson-spine-authoring/reference/beginner-experience-audit.md` |
| Chapter, lesson and section names explain themselves **and carry the owner-supplied curriculum why**, not the subject's own significance | `canon/naming.md` Rule 3, `corrections.md` §18 |
| The old `intro-1`..`intro-5` are a **topic-coverage source** for this stage, never a style model | `canon/corrections.md` §20 |
...
| Light historical register — story and analogy, not citation density | `canon/corrections.md` §4 **(now binding canon, not a non-binding record)** |
| Learn everything a source can teach — structure and tone included — then go further; never reproduce its sentences | `curriculum-state/canon/research-and-comparison.md` |
