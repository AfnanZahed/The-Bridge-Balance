...
**1. Open in the middle of something.** Never a definition, never "in this
lesson". Open on a situation, a question, a moment of friction — the exact
instant the misconception forms. The definition comes after the reader wants it.
...
obvious in retrospect, and it only works in that order.
...
**Gloss every technical term inline at first use, in twelve words or fewer, in
every chapter, every time** — even when an earlier chapter already defined it.
First use is counted per chapter, independently. This does not relax by stage.
`lesson-spine-authoring/reference/mixed-audience.md` §4 has the mechanics, and
the rule that a gloss needing more than twelve words is a foothold, not a gloss.
...
stations** — a Tool chapter leans on How and Cost, a Concept chapter on Why and
What. `lesson-spine-authoring/reference/stations.md` §3 owns that, and it is a
