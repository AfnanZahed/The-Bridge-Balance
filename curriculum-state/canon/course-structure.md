...
**Confirmed again later on 21 September.** The owner: *"yes, one reader, but make it expert. it will be beginner -> Expertise, gradually."* They also said that who reads the last stages *"doesn't matter anymore, because the whole curriculum is going to be redesigned"*, so CS-2 stands for every stage the book produces.
...
**Claude's reading, to confirm as the chapters are authored (not a decision):** a later stage may use what the earlier stages taught, because its reader has finished them. It still explains every new word and never tells any reader to skip.

## 2. The levels of the course today

*The records describe the levels in two slightly different ways, and that was the source of the confusion. The owner settled it on 21 September (CS-8, just below the table). The rows below change as their part of the book is authored (CS-36).*
...
The owner's words: *"Ok, I accept the Stage → Course → Chapter."* For the inside of a chapter, *"Chapter → Lesson → Part (if any)"*. The wording readers are told today ("Lesson or Part", right-hand column above) is replaced by CS-8. The site still says it, and changes as the work is done (section 7).
...
*The names when this file was created, from `thesis.md`. **Superseded 22 September 2026** — see the note under the table.*
...
| # | Decision | Status | Date |
|---|---|---|---|
| CS-5 | **Stage 0 will be named "Introduction to SDE".** | DECIDED (the name). Applied as Stage 0's own pages are written (CS-36). | 21 Sep 2026 |
| CS-6 | **The whole curriculum is being rethought and replanned, not only the names.** The owner: *"Complete change is going to happen in curriculum."* | DECIDED (the direction). **Closed 25 Sep 2026** — the direction (CS-12 to CS-16) is decided and applied, and below the stage level design happens in realtime (CS-36). Until then this cell read "Not started." | 21 Sep 2026 |
...
### The redesign is closed, and design happens in realtime — decided 25 September 2026

| # | Decision | Status | Date |
|---|---|---|---|
| CS-36 | **The redesign is complete, and everything below the stage level is designed in realtime.** The redesign was the direction itself — three stages, with agentic AI taught inside SE (CS-12 to CS-16) — and it is decided and applied. From here, nothing below that is designed in advance: every chapter, lesson, topic and sub-topic — its scope, its place in a stage, and its title — is designed at the moment it is authored, as the book grows (D5, CS-28). No session waits for a redesign, a replan or a chapter list to arrive before authoring. | DECIDED | 25 Sep 2026 |

The owner's decision, 25 September 2026. It also gives locked decision D5 its written home: until now, `CLAUDE.md` and other files cited D5 without saying where it lives. D5's one approved exception is untouched — Stage 0's twenty-chapter ladder in `curriculum-state/stage-0-drafting-plan.md` stays as approved. The topic sketch (CS-29) is also untouched: still a very rough idea, not a plan.

The owner's words: *"the whole curriculum redesign is already completed, since the stages are now 3, mean the main direction is redesigned"*, and *"the exact chapters, and topics will be being designed in realtime"*.
...
Each row is a copy or a claim that this file may overtake. Rows marked **Done** were brought into line on 21 September 2026. The others are changed as their part of the book is authored — each one either brought into line or turned into a pointer to this file. **22 September 2026:** the rows about the four-stage migration are halted, not waiting — the migration was superseded the same day by the Agentic-inside-SE decision (CS-12 to CS-16); see the notes in the rows and `specs/011-curriculum-redesign/tasks.md`.

| File | What it says now | Decision it disagrees with |
|---|---|---|
| `.specify/memory/constitution.md`, Principle IX | **Done (v3.0.2, 21 September).** It says one reader in every stage, a beginner who becomes an expert | Nothing now. It does not name the levels; that is another amendment. |
...
| `.claude/skills/lesson-spine-authoring/` | **Done for the words** (234 changed in 20 files, 21 September). The two folder names still say "lesson" (`lesson-spine-authoring`, `lesson-adversarial-review`) | Only the folder names. Renaming them is one script pass, best done when no other session is open |
...
- **25 September 2026.** CS-36 entered (section 5): the owner closed the redesign — it was the direction itself, and the direction is decided and applied — and confirmed that every chapter, lesson, topic and sub-topic is designed in realtime at the moment it is authored, never in advance (D5, CS-28). D5 now has a written home in this file. The sentences that had the redesign still arriving (CS-6's status, section 2, section 5, CS-Q6 and section 7) carry dated corrections, and the same rule is stated in `corrections.md` §31 and in the D5 bullet of `CLAUDE.md` and `AGENTS.md`. The Agenda was refreshed in the same pass.