 ...
 *   - Palette: single Apple-system blue primary (R-005). Bars share the
 *     semantic `--tbb-accent`; stage identity comes from the `short` label
 *     on the axis + legend, never from color alone (FR-008).
...
export const BAR_CHART_DATA: CurriculumBarDatum[] = [
  { stage: "Spec-Aware Vibe Engineering", short: "Stage 1", chapters: 6, fill: "var(--tbb-accent, oklch(0.5 0.21 264))" },
  { stage: "Credible Validation", short: "Stage 2", chapters: 2, fill: "var(--tbb-accent, oklch(0.5 0.21 264))" },
  { stage: "Mastering AI Coding Agents", short: "Stage 3", chapters: 4, fill: "var(--tbb-accent, oklch(0.5 0.21 264))" },
  { stage: "Engineering Autonomous Agents", short: "Stage 4", chapters: 3, fill: "var(--tbb-accent, oklch(0.5 0.21 264))" },
];

function renderBarTooltip(props: TooltipContentProps<number, string>): React.ReactNode {
...
export default function CurriculumBarChart(): React.ReactElement {
  return (
    <>
      ...
      <LegendStrip items={BAR_CHART_DATA.map((d) => ({ label: d.short, color: d.fill }))} />
    </>
  );
}