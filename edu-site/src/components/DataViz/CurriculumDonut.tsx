 ...
 *   - Single-accent + neutrals palette (R-005): Apple-blue primary segment,
 *     then grayscale steps; stage identity communicated by the explicit legend
 *     labels, never color-only meaning (FR-008).
...
/** Monochromatic steps: Apple-blue primary → neutral grayscale (R-005). */
export const DONUT_PALETTE = [
  "var(--tbb-accent, #0071e3)", // Foundations — primary
...
export const DONUT_DATA: DonutDatum[] = [
  { name: "Spec-Aware Vibe Engineering", short: "Foundations", hours: 20, fill: DONUT_PALETTE[0] },
  { name: "Credible Validation", short: "Credentials", hours: 8, fill: DONUT_PALETTE[1] },
  { name: "Mastering AI Coding Agents", short: "Mastery", hours: 12, fill: DONUT_PALETTE[2] },
  { name: "Engineering Autonomous Agents", short: "Autonomy", hours: 8, fill: DONUT_PALETTE[3] },
];

const TOTAL_HOURS = DONUT_DATA.reduce((sum, d) => sum + d.hours, 0);

function renderDonutTooltip(props: TooltipContentProps<number, string>): React.ReactNode {
  ...
  return (
    <div className="tbb-chart-tooltip" role="tooltip">
      ...
      <div className="tbb-chart-tooltip__sub">Estimated commitment — self-paced</div>
    </div>
  );
}
...
function CustomLegend({
  ...
  return (
    <div className="tbb-chart-legend" role="list" aria-label="Curriculum stages">
      {payload.map((entry) => (
        <span key={entry.value} className="tbb-chart-legend__item" role="listitem">
          <span
        ...
        <PieChart accessibilityLayer>
          ...
          >
            {DONUT_DATA.map((entry) => (
              <Cell key={entry.short} fill={entry.fill} stroke="var(--tbb-surface)" strokeWidth={2} />
            ))}
          </Pie>
