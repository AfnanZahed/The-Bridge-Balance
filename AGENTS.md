...
**A chapter is never just a chapter — it is one step in a long-term plan, not lesson production for a class of students.** The owner learns each topic first-hand, applies it on a real project (this site counts), and then teaches from the book. So every chapter serves four purposes at once: his learning forcing-function, a portfolio built from real applications, the material he teaches from, and part of the storefront for the paid catalog sold beside the book. Keep that whole plan in mind whenever you write or revise a chapter. The projects, and the rule that every chapter names where its knowledge gets applied: `curriculum-state/canon/projects.md`.
...
- **No fixed chapter list or chapter count, ever** (locked decision D5; recorded in `curriculum-state/canon/course-structure.md` — CS-28 and CS-36). The official curriculum fixes three stages and nothing below that — the topic, title, and scope of each chapter is decided at authoring time, in realtime as the book is written, and keeps changing as it grows. The curriculum redesign itself is closed (CS-36): no session waits for a design phase, a replan, or a chapter list to arrive before authoring. Never write or imply a finalized table of contents, a target chapter count, or a "the book is N chapters" claim anywhere in this repo. An earlier draft did exactly that (counted placeholder files on disk and wrote "17 chapters" into four different files); all four were corrected. Don't repeat it. **One approved exception:** the owner approved Stage 0's twenty-chapter ladder in `curriculum-state/stage-0-drafting-plan.md` on 2026-09-16 and reconfirmed it on 2026-09-20. That count is deliberate — leave it alone, and do not extend it to any other stage.
...
- **The community is real people — public image is first-class work, and business calls are made deliberately** (owner rule, 25 Sept 2026; `corrections.md` §32). The students and readers are not an audience to update when convenient: a promised date is a debt, and silence compounds it daily. Recovery, when a line has gone quiet, is receipts — finished things, never another apology cycle. **No public message without goods:** every student- or reader-facing message carries something finished (a link that opens, a video that plays), and no new deadline is written into one — a date may be stated only once the thing exists. **Business decisions** — money, pricing, certificates, platform, community — are made on purpose, researched and written down in their canon home, never drifted into as mid-task defaults. **When picking the frog, weigh the public cost:** an open promise to students, readers or the community outranks internal polish.
- **Every chapter names where its knowledge gets applied — the learn–apply–teach loop** (owner rule, 26 Sept 2026). The owner learns a topic first-hand, applies it on a real project — the book's own website counts, and so does any project or hackathon blueprint — and then teaches from the book. So whenever content is written for any stage, the session checks which project(s) this knowledge can be applied to and names them, with one concrete first application each, in `chapter-production`'s step 9 report, appended to `curriculum-state/ledgers/application-ledger.yaml`. The session proposes; the owner applies. The projects and the full rule: `curriculum-state/canon/projects.md`.
...
2. Pick the frog: the task with the biggest consequences for the goal. When unsure, it is usually one of these:
   ...
   - an open promise to students, readers or the community — silence is the most expensive kind of open loop
