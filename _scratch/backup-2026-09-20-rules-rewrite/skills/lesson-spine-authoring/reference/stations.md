...
**Scope note on dates and sources, added 17 September 2026.** S2, S7 and S9 all ask for dates, figures and named origins, and they are right to for the book's mixed-audience stages. For **Stage 0, 1 and 2** material, `curriculum-state/canon/corrections.md` §4 outranks them: the rigour stays in the research and the ledgers, and the reader-facing prose carries the story and the reason, with a date only where it is load-bearing to the point being made.
...
**FrontDoor** — the fifth shape, added 17 September 2026 after four lesson shapes between them failed to describe a stage's opening page. A FrontDoor teaches none of its stage's material. What it carries is the stage's shape and its promise, and it is the one place in the book where warmth and orientation outrank density.
...
**A stage's first position introduces the stage itself** — what it is, what it covers, in what order, why that order, what it costs the reader, and where it ends. It teaches none of the stage's own material. It has no weighting row of its own: it borrows Concept's, leans on S1, S3, S6 and S10, and drops S5's demonstration arm with a logged reason, because there is nothing in it to run.
