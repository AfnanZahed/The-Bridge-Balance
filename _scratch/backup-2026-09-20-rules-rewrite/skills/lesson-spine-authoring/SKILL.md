...
**The opening.** Before the lesson teaches anything, the reader finds out why it exists *now*: what the previous one left unresolved, what changed to make this subject necessary, and what they are about to meet. **This is content, not a preamble.** It answers a question the reader is genuinely holding, in the lesson's own voice, and it is short. It is never "in this lesson we will explore…" and never a hook written to hold attention the lesson has not earned yet. **It does carry a short orientation**, required from 17 September 2026 on the owner's instruction: what the chapter covers, as a few grouped bullets, and what the reader will hold at the end of it. A preview announces that a chapter is starting; an orientation shows its shape. A reader who cannot see that shape before the teaching starts has to assemble it from the inside while also learning it, and the chapter that forced this rule opened on a traffic light under a bare section heading. `curriculum-state/canon/corrections.md` §7 carries the slot order and the worked example.

Concretely: a lesson on assembly language opens by naming what assembly replaced and what reading raw numbers actually cost the people doing it. A lesson on containers opens on the deployment failure that made them worth inventing. **A lesson that opens directly on its own subject — the first instruction of the thing itself — has skipped the step that makes the subject land**, and the reader has nothing to attach the opening paragraph to.
...
**A stage's first position is an introduction to that stage**, which is a different object from a lesson: what the stage is, what it covers, in what order, why that order, what it costs the reader, and where it ends. It teaches none of the stage's own material. Skip it and every lesson in the stage opens on something the reader has no reason to want yet.
...
**A fifth shape: FrontDoor.** Added 17 September 2026. A stage's opening page, a welcome page, any page a reader meets before they have agreed to read anything. A FrontDoor teaches none of its stage's material — it carries the stage's shape and its promise, and it is the one place in the book where warmth and orientation outrank density. Its station weighting is its own table in `reference/stations.md` §3; it drops S0, S5, S9 and S10 on purpose, and `curriculum-state/canon/corrections.md` §1 is the rule it exists to enforce.
...
**Academic furniture in reader-facing prose.** Dates, full names, institutions and citations stacked into teaching prose because the research turned them up. The research stays rigorous; the chapter does not display it. Five rules here pull the other way — Principle 7's "real dates", S2's "state the date", S7's "date every figure", S9's originating work, and `canon/thesis.md`'s year-inline rule — and for Stage 0 to 2 material `curriculum-state/canon/corrections.md` §4 outranks all five. Satisfy the station with the fact, not with the citation apparatus.

**The bolded lead sentence.** Paragraph after paragraph opening on a bolded claim, until bold marks nothing at all. `language-register.md` §6.
...
- [ ] The lesson opens by connecting backwards — no cold open on its own subject, and the first paragraph answers a question the reader is already holding
- [ ] The closing names what the lesson left unresolved and walks the reader to the next one, after the retrieval prompt
- [ ] A stage's first position introduces the stage itself — what it is, what it covers, in what order, why that order, what it costs, where it ends — and teaches none of its material
