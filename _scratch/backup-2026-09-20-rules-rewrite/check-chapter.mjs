 ...
 * orientation, date-density and bold-lead checks below and from nothing else.
...
async function checkChapter(file) {
  ...
  // --- orientation, date density, bold rhythm ------------------------------
  // canon/corrections.md §7 (a chapter opens with orientation, not with its
  // own subject), §4 (historical material stays light in reader-facing prose),
  // and language-register.md §6's bolded-lead-sentence tic. All three shipped
  // inside text-ready chapters while this gate reported clean, which is the
  // whole reason they are checked mechanically rather than left to a read.
  ...
  if (h1Match && !isLegacy) {
    ...
    if (preWords === 0) {
      ...
      push(
        warnings,
        "orientation-thin",
        `the opening orientation has ${why} - §7 asks for why-now, what the chapter covers as grouped bullets, and what the reader ends holding`,
      );
    }
  }
  ...
  if (!isLegacy && proseParas.length >= 8) {
    ...
    if (pct > 40) {
      push(
        warnings,
        "bold-lead",
        `${boldLed} of ${proseParas.length} paragraphs open on a bolded claim (${pct.toFixed(0)}%) - past roughly a quarter, bold marks nothing (language-register.md §6)`,
      );
    }
  }
