...
- **Crash Course CS (2017):** separates the *history* episode from the *mechanism* episodes — history as on-ramp, mechanism as payload. **Flag:** this book's Stage 0 deliberately inverts that (history *is* the spine), so only borrow the episode-level clarity, not the sequencing.
