...
**Check who the chapter is for before the first sentence.** Who each stage is written for — absolute beginners in Stages 0, 1 and 2, and nobody else — is settled in `curriculum-state/canon/audience.md`, "The reader"; the correction record is `curriculum-state/canon/corrections.md`, "8. Stages 0–2 are written for absolute beginners, and for nobody else".

*The two-reader constraint, and every rule built on keeping a senior engineer rewarded, was withdrawn by the owner on 20 September 2026 and is not applied.* The rule, and the named list of withdrawn rules, is `curriculum-state/canon/corrections.md`, "8. Stages 0–2 are written for absolute beginners, and for nobody else"; where the withdrawal binds a draft is `curriculum-state/canon/audience.md`, "The reader".
...
`reference/mixed-audience.md` holds the reader model and the foothold / depth-block layering. The requirement that every Stage 0–2 draft passes the ten-angle beginner check in `reference/beginner-experience-audit.md` before it is presented is `curriculum-state/canon/audience.md`, "The reader".
...
**For The Bridge Balance:** `curriculum-state/canon/research-and-comparison.md` is the authoritative statement, and this skill points there rather than restating it. The Stage 2 integrity floor that overrides it wherever CS50 material is the source being studied is `curriculum-state/canon/integrity-floor.md`, "Known live conflict — Stage 2, CS50" — the boundary is checked first and any conflict surfaced, never worked around.
...
**Scope, Stages 0–2.** A number reaches the page when it changes what the reader understands, in a form they can feel — a comparison, a scale, a "that is about the size of" — not as a dated citation; the rest stays in the ledgers. The rule is `curriculum-state/canon/corrections.md`, "11. Research makes the writer correct. It is not the reader's diet". The old wording, "numbers are not optional", was one of the rules that turned beginner chapters into reference pages.
...
same file), and the stage table is re-derived from the official doc's four stage
sections ("The stages").
...
- `reference/beginner-experience-audit.md` — will a beginner *stay*? Required on all Stage 0–2 material — `curriculum-state/canon/audience.md`, "The reader". The comprehension audit checks none of this, which is why both the rejected Stage 0 intro and the cold `ch01` passed it clean.
