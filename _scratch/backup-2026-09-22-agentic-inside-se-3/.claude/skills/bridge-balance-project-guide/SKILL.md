...
7. **Stages 0–2 are written for absolute beginners, and for nobody else.** *(Replaces the two-reader rule, withdrawn by the owner on 2026-09-20 — `corrections.md` §8.)* The reader has never programmed, does not know what Python is, and may not be sure what a terminal is for. There is no second, expert reader sharing the page, and no rule may be justified by keeping one engaged. Stages 3–4 have the same reader, further along the road (`curriculum-state/canon/audience.md`, "The later stages"); how they are written waits for the redesign, and no separate senior reader may be assumed. `curriculum-state/canon/audience.md` is the canon statement. What binds every task:
...
That table assumes the topic already has an identity — an existing placeholder file, or at least a row in the prerequisite graph. Most Stage 1–4 subtopics already do. If someone asks for a chapter on something genuinely new that isn't in either yet, start with `reference/curriculum-architect/SKILL.md` instead, even though the request sounds like a `chapter-production` job — it's what gives a new topic a place in the graph before there's anything for the other protocols to act on.
