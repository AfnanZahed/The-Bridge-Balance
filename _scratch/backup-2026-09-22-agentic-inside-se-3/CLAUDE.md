...
- **The shape of the course is decided in one file.** `curriculum-state/canon/course-structure.md` records who the reader is, the levels (Stage → Course → Chapter → Lesson → Part), the learning-first naming rule, the level labels, and the direction of the redesign now under way (learning is parallel; the old Stages 1 and 3 merge; Stage 0 is named "Introduction to SDE"). **Read it before naming, renaming, splitting or planning any stage, course, chapter or lesson, and before writing any sentence about who the book is for.** The whole curriculum is being redesigned (decided 2026-09-21), so do not treat the stage list in this file as final. Decisions of this kind are changed in that file first.
