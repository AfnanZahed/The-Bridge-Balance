...
stage of *the book* is about). The stage that holds the tool list draws from
both: the tool list is quoted from `the solution`, the discipline list is
...
| 1 | SDE Mastery (AI-Driven) | Architecture, dev environment, core programming, packages, OOP, Git/GitHub, frontend, UI/UX, backend, auth, async/realtime, databases, personal branding — learned together with coding agents like Claude Code and OpenCode, prompt / context / loop engineering, CLAUDE.md and AGENTS.md, skills, MCP servers and workflows. The old Stage 1 and old Stage 3, merged 2026-09-22. |
| 2 | Credentials | Harvard CS50P and CS50W today. Further free-first certifications are the owner's plan and not yet scoped. |
| 3 | SDE Mastery (AI-Native; Agentic AI) | AI agents, RAG, tool calling, task automation, LLM integration, evaluations, MCP, frameworks (OpenAI Agents SDK, Claude Agents SDK, LangGraph), multi-agent systems. The old Stage 4, renumbered 2026-09-22. |
