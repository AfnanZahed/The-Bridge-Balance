...
**`lesson-spine-authoring` is different — a peer skill, not a bundled protocol.** It lives at `.claude/skills/lesson-spine-authoring/`, carries its own description, and does trigger on its own. That is deliberate: it is the general teaching framework (the Name-First spine, the beginner-first layering, the language register) and it is not specific to this book. The split:
...
7. **Stages 0–2 are written for absolute beginners, and for nobody else.** *(Replaces the two-reader rule, withdrawn by the owner on 2026-09-20 — `corrections.md` §8.)* The reader has never programmed, does not know what Python is, and may not be sure what a terminal is for. There is no second, expert reader sharing the page, and no rule may be justified by keeping one engaged. The audience for Stages 3–4 is undecided; do not assume one. `curriculum-state/canon/audience.md` is the canon statement. What binds every task:
...
└── reference/stations.md            the twelve stations — read in full, first
    ...
    reference/beginner-experience-audit.md  ten angles on whether a beginner
                                     will actually stay — Stages 0–2, required
```
