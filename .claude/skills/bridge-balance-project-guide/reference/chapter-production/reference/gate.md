...
**Nothing in this gate asks you to make or rebuild a picture.** How each image is made is the owner's choice (`corrections.md` §28), and every image is referenced as an ordinary markdown image. The gate checks that a referenced image exists and that its alt text is real prose — nothing more.
...
| `date-density` | Years per thousand words above the Stages 0–1 threshold. Not every date is wrong; the check asks you to look. For each one: is it load-bearing to the sentence carrying it? `corrections.md` §4 |
