...
| T015 | Agent instructions and project docs (inventory category A), with the constitution's MINOR version bump | DeepSeek | ✅ done | `_scratch/job-briefs/2026-09-22/job9-agent-docs.md`; 21 files in inventory category A updated; the constitution 3.0.2 → 3.1.0 (MINOR, amended 2026-09-22); backups in `_scratch/backup-2026-09-22-agentic-inside-se-3/` |
...
| T017 | The site structure pass (inventory category C) | DeepSeek | ✅ done | `_scratch/job-briefs/2026-09-22/job11-site-pass.md`; `stage-03-sde-mastery-ai-native/` → `stage-02-sde-mastery-ai-native/` and `stage-02-credentials/` → `edu-site/parked/credentials-track/` (moves listed in `_scratch/backup-2026-09-22-agentic-inside-se-6/MOVES.txt`); the five hand-written stage lists, the stage icons, `custom.css`, the moved chapters' frontmatter and the ledgers updated; `npm run build` green, `chapterManifest.ts` and `search-index.json` regenerated by the build, never hand-edited |
| T018 | Minimal fact fixes on the reader pages the inventory lists | Claude writes, DeepSeek applies | ✅ done | `welcome.md`, `faq.md`, `ch00-introduction.md`, `ch02-programming-is-born.md` line 74, `code-of-conduct.md`, the two Stage 0 intro chapters that still said "Stage 3", and the root `README.md` now carry the three-stage facts; every CS50 promise left word for word |
| T019 | One changelog entry | Claude | ✅ done | one entry dated 2026-09-22 in `edu-site/docs/changelog.md`: the three stages named, agentic AI inside SE, the CS50 credential pages off the site while the Credentials track is redesigned |
| T020 | The final gate | Claude | ✅ done | `npm run build` green; `npm run typecheck` clean and `npm run lint` at 64 errors / 3 warnings / 10 infos, both exactly the baseline; `node scripts/check-chapter.mjs` 0 errors; the repo-wide grep's remaining hits are dated records, CS50 promises or parked paths |
...
- [x] Every category-A hit fixed in its file's own voice, or marked with a dated note
- [x] `CLAUDE.md`, `PROJECT-MAP.md`, `stack.md`, `.specify/memory/constitution.md`, `.claude/**`, `edu-site/README.md` covered
- [x] The constitution's Sync Impact Report updated, the version bumped MINOR, last-amended 2026-09-22
- [x] CS50 promises, literal stage folder paths, dated history entries and folder/skill names left alone and listed in the report
...
- [x] `edu-site/docs/stage-03-sde-mastery-ai-native/` renamed to `stage-02-sde-mastery-ai-native/`
- [x] `edu-site/docs/stage-02-credentials/` moved out of the published docs to `edu-site/parked/credentials-track/`
- [x] The five hand-written stage lists changed together (their files are named in category C of the inventory), plus the stage icons
- [x] `--tbb-stage-3` and `--tbb-stage-4` retired or renumbered in `edu-site/src/css/custom.css`
- [x] Folder paths updated in `curriculum-state/ledgers/**`
- [x] `chapterManifest.ts` and `search-index.json` rebuilt by `npm run build`, never hand-edited
- [x] `npm run build` green
- [x] No link or path points to an old stage folder

**T018 — the reader pages.**
- [x] `edu-site/docs/welcome.md`, `edu-site/docs/faq.md`, the Stage 0 introduction and `edu-site/docs/ch02-programming-is-born.md` line 74 ("Stages 3 and 4") carry the three-stage facts
- [x] CS50 promises untouched

**T019 — the changelog.**
- [x] One entry in `edu-site/docs/changelog.md`: agentic AI moves inside SE, and the course goes from four stages to three

**T020 — the final gate.**
- [x] `npm run build` green
- [x] `node scripts/check-chapter.mjs` clean
- [x] A repository-wide grep for the old stage names where every remaining hit is a dated record, a CS50 promise or history
