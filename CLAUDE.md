...
- Real stage folders today: `edu-site/docs/stage-01-spec-aware-vibe-engineering/`, `stage-02-cs50-certification/`, `stage-03-mastering-ai-coding-agents/`, `stage-04-engineering-autonomous-ai-agents/`, plus the Stage 0 sequence at the docs root (`intro-1-binary-to-programming.md` through `intro-5-spec-driven-engineering.md`). Current status, per `curriculum-state/ledgers/prerequisite-graph.yaml`: the stage index pages, every Stage 0 chapter, and every Stage 1 chapter are `text-ready`; Stage 2, 3, and 4 remain `placeholder` beyond their index pages. The Stage 0 sequence, though text-ready, is mid-redesign into an officially-promoted, from-scratch chronological Stage 0 (see `curriculum-state/proposals/whole-book-redesign-record-2026-09-14.md`), so its current text should not be treated as final.
...
- **Repo root has no `.git`.** `Book/` is a single working copy (see "No version control in the working process" above). `edu-site/README.md` and `.github/workflows/ci.yml` still describe an earlier GitHub-PR / Vercel-CI deploy flow — that flow is not in effect and shouldn't be suggested or run; it predates the current no-version-control decision.
